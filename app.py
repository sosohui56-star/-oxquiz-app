...
def connect_to_gspread() -> gspread.Client | None:
    ...
    try:
        ...
        st.session_state.sheet_log_status = f"📛 Google Sheets 연결 오류: {e}"
        return None
...
def log_to_sheet(data: dict):
    row = [
        ...
        str(data.get("worksheet_name") or ""), # 워크시트 이름 추가
    ]
    try:
        sheet = connect_to_sheet()
        if sheet is None:
            # 연결 실패 메시지는 main_page에서 st.session_state.sheet_log_status를 통해 표시됨
            return
        sheet.append_row(row)
        st.session_state.sheet_log_status = "✅ 구글 시트에 기록 성공!"
    except Exception as e:
        st.session_state.sheet_log_status = f"📛 구글 시트 기록 실패: {e}"
        # 디버깅을 위해 상세 오류를 앱 화면에 표시
...
def main_page() -> None:
    ...
    if st.sidebar.button("문제집 로드"):
        with st.spinner("문제집을 불러오는 중..."):
            ...
            if not df_source.empty:
                ...
                st.session_state.selected_worksheet_name = worksheet_name if worksheet_name else "Sheet1" # 워크시트 이름 저장
    ...
    if st.session_state.df is not None and not st.session_state.df.empty:
        ...
        if "단원명" in st.session_state.df.columns:
            ...
            filtered_df = st.session_state.df[st.session_state.df["단원명"] == selected_chapter] if selected_chapter != "전체 보기" else st.session_state.df
        else:
        ...
        if st.session_state.answered:
            ...
            answer_buttons = { "O": col1.button("⭕ O", use_container_width=True), "X": col2.button("❌ X", use_container_width=True), "모름": col3.button("⁉️ 모름", use_container_width=True) }
            for user_answer, is_clicked in answer_buttons.items():
                if is_clicked:
                    ...
                    if correct:
                        ...
                        st.session_state.wrong_list.append({"이름": st.session_state.user_name, "날짜": datetime.now().strftime("%Y-%m-%d %H:%M"), "문제번호": qnum_display, "단원명": question.get("단원명", ""), "문제": question["문제"], "정답": question["정답"], "선택": user_answer, "해설": question.get("해설", "") if pd.notna(question.get("해설", "")) else "" })
